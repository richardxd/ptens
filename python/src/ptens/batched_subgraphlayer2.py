...
class batched_subgraphlayer2(p.batched_subgraphlayer,p.batched_ptensorlayer2):
    ...
    def __new__(cls,G,S,atoms,M):
        ...
        R=super().__new__(batched_subgraphlayer2,M)
        R.atoms=atoms
        R.G=G
        R.S=S
        return R
    ...
    def __getitem__(self,i):
        ...
        return p.subgraphlayer2.from_matrixA(self.G[i],self.S,self.atoms[i],M)
    ...
    @classmethod
    def linmaps(self,x):
        return batched_subgraphlayer2(x.G,x.S,x.atoms,super().linmaps(x))
    ...
    def gather(self,S,x):
        ...
        return batched_subgraphlayer2(x.G,x.S,atoms,super().gather(atoms,x))
