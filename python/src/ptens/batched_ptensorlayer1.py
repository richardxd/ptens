...
class batched_ptensorlayer1(p.batched_ptensorlayer):

    @classmethod
    def make(self,atoms,M):
        R=batched_ptensorlayer1(M)
        R.atoms=atoms
        return R
    ...
    def from_ptensorlayers(self,list):
        for a in list:
            assert isinstance(a,p.ptensorlayer1)
    ...
    def backend(self):
        return pb.batched_ptensors1.view(self.atoms,self)
    ...
    def __getitem__(self,i):
        assert i<len(self)
        offs=self.atoms.offset1(i)
        n=self.atoms.nrows1(i)
        M=torch.Tensor(self)[offs:offs+n,:]
        return p.ptensorlayer1.from_matrix(self.atoms[i],M)
    ...
    @classmethod
    def linmaps(self,x):
        return batched_ptensorlayer1_linmapsFn.apply(x)
    ...
    def gather(self,atoms,x,*args):
        ...
        return batched_ptensorlayer1_gatherFn.apply(atoms,x,map)
    ...
    def __repr__(self):
        return "batched_ptensorlayer1(size="+str(len(self))+",nc="+str(self.size(1))+")"
...
class batched_ptensorlayer1_linmapsFn(torch.autograd.Function):
...
class batched_ptensorlayer1_gatherFn(torch.autograd.Function):
    ...
    def backward(ctx,g):
        x, = ctx.saved_tensors
        r = x.zeros_like()
        g_view = pb.batched_ptensors1.view(ctx.atoms, g)
